...
Average_process_details *RR(Process_detail *pd, int no_of_process, int time_quantum) {
  ...
  for (int i = 0; i < no_of_process; i++) {
    if (current_time < pd_RR[i].arrival_time) {
      total_idle_time += (pd_RR[i].arrival_time - current_time);
      current_time = pd_RR[i].arrival_time;
    }
    ...
    while (completed < no_of_process) {
      ...
      int execution_slice = (remaining_time[current_process] < time_quantum) ? 
